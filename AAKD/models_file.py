...
class MoleculeGIN(nn.Module):
    def __init__(self, input_dim=46, hidden_dim=None, output_dim=None, num_layers=None, dropout=None):
        super(MoleculeGIN, self).__init__()
        hidden_dim = hidden_dim or config.GIN_HIDDEN_DIM
        output_dim = output_dim or config.GNN_OUTPUT_DIM
        num_layers = num_layers or config.GIN_NUM_LAYERS
...
class WeightedADRRGCN(nn.Module):
    def __init__(self, input_dim=None, hidden_dim=None, output_dim=64, num_relations=None, dropout=None):
        super(WeightedADRRGCN, self).__init__()
        input_dim = input_dim or config.RGCN_INPUT_DIM
        hidden_dim = hidden_dim or config.RGCN_HIDDEN_DIM
        num_relations = num_relations or config.NUM_RELATIONS
        dropout = dropout or config.DROPOUT_RATE
...
class MoE_Expert(nn.Module):
    def __init__(self, input_dim, hidden_dim=None, output_dim=None, dropout=None):
        super().__init__()
        hidden_dim = hidden_dim or config.MoE_HIDDEN_DIM
        output_dim = output_dim or config.MoE_EXPERT_OUTPUT_DIM
        dropout = dropout or config.DROPOUT_RATE
...
class DynamicPromptCrossAttention(nn.Module):
    def __init__(self, drug_dim=288, adr_dim=256, hidden_dim=None,
                 num_heads=None, num_prompts=None, dropout=None):
        super().__init__()
        hidden_dim = hidden_dim or config.ATTENTION_HIDDEN_DIM
        num_heads = num_heads or config.ATTENTION_NUM_HEADS
        num_prompts = num_prompts or config.ATTENTION_NUM_PROMPTS
        dropout = dropout or config.DROPOUT_RATE
...
class SimpleMLPModel(nn.Module):
    def __init__(self, fp_dim, adr_feature_dim, atc_feature_dim, 
                 embed_dim, atc_l2_list, mol_graphs, dropout=None): 
